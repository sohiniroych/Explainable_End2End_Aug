...
# extract_patches.py
from extract_patches_mod import *
...
def evalResult(gths,pred_imgs,flag_multi_class = False,num_class = 2):
    ff=np.shape(gths)
    n_i=ff[0]
    prec=0
    rec=0
    ...
    f1_score=0
    for i in range(n_i):
        img = np.reshape(pred_imgs[i,0,:,:],(584,565))
        #print(files[i])
        gth = np.reshape(gths[i,0,:,:],(584,565))
...
def test_u_net():
    ...
    if average_mode == True:
        ...
        patches_imgs_test, patches_masks_test = get_data_testing(
            DRIVE_test_imgs_original = DRIVE_test_imgs_original,  #original
            DRIVE_test_groudTruth = path_data + config.get('data paths', 'test_groundTruth'),  #masks
            DRIVE_test_border_masks=path_data + config.get('data paths', 'test_border_masks'),  #masks
            Imgs_to_test = int(config.get('testing settings', 'full_images_to_test')),
            patch_height = patch_height,
            patch_width = patch_width,
        )
    ...
    file_perf.close()
    evalResult(gtruth_masks,pred_imgs)
