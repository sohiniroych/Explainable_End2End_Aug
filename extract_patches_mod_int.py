...
#Modify this function to allow larger tiles [100x100]
...
def get_data_training(DRIVE_train_imgs_original,
    ...
    patches_imgs_train, patches_masks_train, patches_borders_train = extract_ordered_train(train_imgs,train_masks,train_borders,patch_height,patch_width)
...
#Load the original data and return the extracted patches for training/testing
    #Make changes to this as well
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth, DRIVE_test_border_masks,Imgs_to_test, patch_height, patch_width):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
    test_masks = load_hdf5(DRIVE_test_groudTruth)
    test_borders = load_hdf5(DRIVE_test_border_masks)


    test_imgs = my_PreProc(test_imgs_original)
    test_masks = test_masks/255.
    test_borders=test_borders/255.

    #extend both images and masks so they can be divided exactly by the patches dimensions
    test_imgs = test_imgs[0:Imgs_to_test,:,:,:]
    test_masks = test_masks[0:Imgs_to_test,:,:,:]
    test_borders=test_borders[0:Imgs_to_test,:,:,:]
    test_imgs = paint_border(test_imgs,patch_height,patch_width)
    test_masks = paint_border(test_masks,patch_height,patch_width)
    test_borders = paint_border(test_borders,patch_height,patch_width)

    data_consistency_check(test_imgs, test_masks,test_borders)
    ...
    #extract the TEST patches from the full images, non overlapping
    patches_imgs_test = extract_ordered(test_imgs,patch_height,patch_width)
    patches_masks_test = extract_ordered(test_masks,patch_height,patch_width)
    patches_borders_test=extract_ordered(test_borders,patch_height,patch_width)
    data_consistency_check(patches_imgs_test, patches_masks_test,patches_borders_test)
...
def data_consistency_check(imgs,masks,borders):
    assert(len(imgs.shape)==len(masks.shape))
    assert(imgs.shape[0]==masks.shape[0])
    assert(imgs.shape[2]==masks.shape[2])
    assert(imgs.shape[3]==masks.shape[3])
    assert(masks.shape[1]==1)
    assert(borders.shape[1]==1)
    assert(imgs.shape[1]==1 or imgs.shape[1]==3)


#extract patches randomly in the full training images
#  -- Inside OR in full image
def extract_random(full_imgs,full_masks, full_borders,patch_h,patch_w, N_patches, inside=True):
    ...
    assert (full_borders.shape[1]==1)
    assert (full_imgs.shape[2] == full_masks.shape[2] and full_imgs.shape[3] == full_masks.shape[3])
    assert (full_imgs.shape[2] == full_borders.shape[2] and full_imgs.shape[3] == full_borders.shape[3])
    patches = np.empty((N_patches,full_imgs.shape[1],patch_h,patch_w))
    patches_masks = np.empty((N_patches,full_masks.shape[1],patch_h,patch_w))
    patches_borders = np.empty((N_patches,full_borders.shape[1],patch_h,patch_w))
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        ...
        while k <patch_per_img:
            ...
            patch_border=full_borders[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]
            if inside==True:
                if is_patch_inside_FOV_mod(patch_border)==False:
                    continue
            patches[iter_tot]=patch
            patches_masks[iter_tot]=patch_mask
            patches_borders[iter_tot]=patch_border
            iter_tot +=1   #total
            k+=1  #per full_img
    return patches, patches_masks, patches_borders
...
def is_patch_inside_FOV_mod(border):
    pix=np.sum(border[:,:])
    den=np.prod(np.shape(border))
    if pix/den >=0.9:
        return True
    else:
        return False    
...
def extract_ordered_train(full_imgs, full_masks,full_borders,patch_h, patch_w):
    assert (len(full_imgs.shape)==4 and len(full_masks.shape)==4)  #4D arrays
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  #check the channel is 1 or 3
    assert (full_masks.shape[1]==1)   #masks only black and white
    assert (full_borders.shape[1]==1)
    ...
    N_patches_tot = (N_patches_h*N_patches_w)*full_imgs.shape[0]
    patches = np.empty((N_patches_tot,full_imgs.shape[1],patch_h,patch_w))
    patches_masks = np.empty((N_patches_tot,full_masks.shape[1],patch_h,patch_w))
    patches_borders = np.empty((N_patches_tot,full_borders.shape[1],patch_h,patch_w))
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        for h in range(N_patches_h):
            for w in range(N_patches_w):
                patch = full_imgs[i,:,h*patch_h:(h*patch_h)+patch_h,w*patch_w:(w*patch_w)+patch_w]
                mask = full_masks[i,:,h*patch_h:(h*patch_h)+patch_h,w*patch_w:(w*patch_w)+patch_w]
                border=full_borders[i,:,h*patch_h:(h*patch_h)+patch_h,w*patch_w:(w*patch_w)+patch_w]
                patches[iter_tot]=patch
                patches_masks[iter_tot]=mask
                patches_borders[iter_tot]=border
                iter_tot +=1   #total
    assert (iter_tot==N_patches_tot)
    return patches, patches_masks, patches_borders
...
#Extend it to path, mask, border for training
...
def extract_ordered_overlap_train(full_imgs, full_masks, full_borders,patch_h, patch_w,inside=True):
    stride_h=32
    stride_w=32
    assert (len(full_imgs.shape)==4)  #4D arrays
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  #check the channel is 1 or 3
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        for h in range((img_h-patch_h)//stride_h+1):
            for w in range((img_w-patch_w)//stride_w+1):
                patch = full_imgs[i,:,h*stride_h:(h*stride_h)+patch_h,w*stride_w:(w*stride_w)+patch_w]
                patch_mask = full_masks[i,:,h*stride_h:(h*stride_h)+patch_h,w*stride_w:(w*stride_w)+patch_w]
                ...
                iter_tot +=1   #total
    #assert (iter_tot==N_patches_tot)
    return patches,patches_masks,patches_borders  #array with all the full_imgs divided in patches
...
def recompone(data,N_h,N_w):
    ...
    N_patch_per_img = int(N_w*N_h)
    assert(data.shape[0]%N_patch_per_img == 0)
    N_full_imgs = int(data.shape[0]/N_patch_per_img)
    patch_h = data.shape[2]
    patch_w = data.shape[3]
    #N_patch_per_img = int(N_w*N_h)
    #define and start full recompone
    #print(N_full_imgs,N_h*patch_h,N_w*patch_w)
